...
from scipy.signal import fftconvolve, argrelextrema, lfilter
...
def get_center_from_stations(stations):
    ...
    return ortho.geographic_midpoint(slats, slons)


def get_theoretical_backazimuth(event, stations):
    center_lat, center_lon = get_center_from_stations(stations)
    return (ortho.azimuth_numpy(event.lat, event.lon, center_lat, center_lon)\
            + 180.) % 360.


def get_shifts(stations, bazis, slownesses):
    ''' shape = (len(bazi), len(slow))'''
    lats = num.array([s.lat for s in stations])
    lons = num.array([s.lon for s in stations])
    center_lat, center_lon = ortho.geographic_midpoint(lats, lons)
    lat0 = num.array([center_lat] * len(stations))
    lon0 = num.array([center_lon] * len(stations))
...
class FK(Snuffling):
    ...
    specific maximum of that processing block.
        ...
        for traces in self.chopper_selected_traces(
            ...
            for itr, tr in enumerate(traces):
                if taper is not None:
                    ydata = fftconvolve(arrays[itr], taper)
                else:
                    ydata = arrays[itr]
            ...
            times = num.linspace(t_min, t_max, lengthout)
            ...
            #axs = []
            #for i in range(3):
            #    axs.append(fig1.add_subplot(3, 1, i+1))
            #print self.win_length
            #print self.win_length * deltat_cf
            #plot_back_slow_time(
            ...
            if self.want_all:
                ...
                ax_raw = fig1.add_subplot(3, 1, 1)
                ax_shifted = fig1.add_subplot(3, 1, 2)
                ax_beam = fig1.add_subplot(3, 1, 3)
                ...
                for i, (shift, array) in enumerate(zip(shifts.T, arrays)):
                    ...
                    ybeam += traces[i].get_ydata()[npad-ishift: -npad-ishift]
                ...
                beam_tr = trace.Trace(tmin=t_min, ydata=ybeam, deltat=deltat_cf)
