...
c
      character*40  ic*80, file_check*80
...
c
      ...
      phdif(9)='sPKPab'
c++
      call asnag1(nin,-1,1,requ,modnam)
c++ 
...
c
c changes for NORSAR Jul 16, 1997 JS
c
      ic =  modnam(1:nb) // '.tbl'

      ic = file_check (ic) 

      call dasign(nin,-1,ic,nasgr)
...
c206  format(/(1x,i5,2f12.6))
      ...
      real usrc(2)
c     double precision pm,zm,us,pt,tau,xlim,xbrn,dbrn,zs,pk,pu,pux,tauu,
      double precision us,pt,tau,xlim,xbrn,dbrn,zs,pk,pu,pux,tauu,
     1 xu,px,xt,taut,coef,tauc,xc,tcoef,tp
      common/tabc/us(2),pt(jout),tau(4,jout),xlim(2,jout),xbrn(jbrn,3),
     1 dbrn(jbrn,2),xn,pn,tn,dn,hn,jndx(jbrn,2),idel(jbrn,3),mbr1,mbr2
      common/brkc/zs,pk(jseg),pu(jtsm0,2),pux(jxsm,2),tauu(jtsm,2),
...
cjs   data tol,dtol,deplim,ka,lpower/.01,1d-6,1.1,4,7/
...
c
 58   du=dble(amin1(1e-5+(odep-.4)*2e-5,1e-5))
...
cj.s. double precision function umod(zs,isrc,nph)
      function umod(zs,isrc,nph)
      double precision umod
      save 
      include 'ttlim.h'
      character*31 msg
...
c
      save
cj.s.      double precision buf(len),tmp
...
c     double precision pm,zm,us,pt,tau,xlim,xbrn,dbrn,zs,pk,pu,pux,tauu,
     ...
     5 jidx(jbrn),kk(jseg)
      common/pcdc/phcd(jbrn)
      data dbrnch,cn,x180,x360,dtol,ptol/2.5307274d0,57.295779d0,
     1 3.1415927d0,6.2831853d0,1d-6,2d-6/
...
c
 ...
 7    tau(2,i)=coef(3,i)*b(j-1)+coef(4,i)*b(j)+coef(5,i)*b(j+1)
      ...
      subroutine trtm(delta,n,tt,dtdd,dtdh,phnm)
      save 
      include 'ttlim.h'
      character*(*) phnm(maxp)
      character*8 ctmp(maxp)
      dimension tt(maxp),dtdd(maxp),dtdh(maxp),dddp(maxp),tmp(maxp,4),
     1 iptr(maxp)
...
c
 ...
 9    if(dabs(x(1)-pi).gt.dtol) go to 7
      ...
      x(2)=-10d0
 7    do 1 j=mbr1,mbr2
 1    if(jndx(j,2).gt.0) call findtt(j,x,n,tmp,tmp(1,2),tmp(1,3),
     1 tmp(1,4),ctmp)
      if(n-1)3,4,5
 4    iptr(1)=1
      go to 6
...
c
c     old syntax
cj.s.      if(iargc(i).lt.n) go to 1
c

c
c    To avoid problems with calling routines
c    which also have an argument list.
c
c    Jul 16, 1997 NORSAR Johannes Schweitzer
c
cjs      if(iargc().lt.n) go to 1
cjs      call getarg(n,ib)
cjs      go to 2
c
 1    if(ib.ne.' ') go to 2
      call query(ia,log)
...
c
c changed for  NORSAR Jul 16, 1997 JS
c
...
c
      ...
      go to (1,2),nf
 1    go to (11,12,13),ns
 11   open(lu,file=ia(1:na),status='old',form='formatted')
      rewind lu
      return
 12   inquire(file=ia(1:na),exist=exst)
      if(exst) go to 11
 13   open(lu,file=ia(1:na),status='new',form='formatted')
      return
 2    go to (21,22,23),ns
 21   open(lu,file=ia(1:na),status='old',form='unformatted')
      rewind lu
      return
 22   inquire(file=ia(1:na),exist=exst)
      if(exst) go to 21
 23   open(lu,file=ia(1:na),status='new',form='unformatted')
      return
      end
...
c      P      gives P-up, P, Pdif, PKP, and PKiKP
c      P+     gives P-up, P, Pdif, PKP, PKiKP, PcP, pP, pPdif, pPKP,
c             pPKiKP, sP, sPdif, sPKP, and sPKiKP
c      S+     gives S-up, S, Sdif, SKS, sS, sSdif, sSKS, pS, pSdif,
c             and pSKS
c      basic  gives P+ and S+ as well as ScP, SKP, PKKP, SKKP, PP, and
c             P'P'
c   Note that generic S gives S-up, Sdif, and SKS already and so
...
c   Print the result.
 20   j2=j2-1
      if(.not.fnd.and.phlst(1).eq.'query') then
        print *,'Brnset:  the following phases have been selected -'
...
c     double precision us,pt,tau,xlim,xbrn,dbrn,zs,pk,pu,pux,tauu,xu,
...
c
      ...
      phsyn=phnm
c
c changed and added to get the right PKP branch J.S.
c
c     j=index(phsyn,'bc')
...
c
 ...
 100  format(a,$)
      ...
      subroutine dasign(lu,mode,ia,leng)
c
c $$$$$ calls no other routine $$$$$
c
